...
module.exports.run = async (client, message, args) => {
    ...
    switch (args[0]) {
        case "set": //switch(args[1]){
            if (!modes[args[1]]) return message.reply(`Aviable Modes:\n1 = **Nova (Default)**\n2 = **Thundero**`)
            if (client.permlvl != 5) {
                if (!modes[args[1]].public) {
                    if (modes[args[1]].owner.id !== message.author.id) return message.channel.send(`It doesnt look like you are ${modes[args[1]].owner.name}`)
                    let mode = Number(args[1])
...
module.exports.help = {
    name: "mode",
    permlvl: 3,
    description: "Change Nova Mode - Premium",
    hidden: false,
    aliases: [],
